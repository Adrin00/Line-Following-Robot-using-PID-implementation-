...
void loop()
{
  read_sensor_values();
  Serial.print(error);
  if (error == 100) {               // Make left turn untill straight path is found
    do {
      read_sensor_values();
  ...
  } else if (error == 101) {          // Make right turn in case it detects only right path untill straight path is found.
    analogWrite(ENA, 110); //Left Motor Speed
    analogWrite(ENB, 90); //Right Motor Speed
    forward();
    delay(200);
  ...
  } else if (error == 102) {        // Make left turn untill straight path is found
    do {
      analogWrite(ENA, 110); //Left Motor Speed
      analogWrite(ENB, 90); //Right Motor Speed
      sharpLeftTurn();
  ...
  } else if (error == 103) {        // Make left turn untill it detects straight path or stop if reaches dead-end.
    if (flag == 0) {
      analogWrite(ENA, 110); //Left Motor Speed
      analogWrite(ENB, 90); //Right Motor Speed
      forward();
...
void motor_control()
{
  //the effective motor speed calculation
  int left_motor_speed = initial_motor_speed - PID_value;
  int right_motor_speed = initial_motor_speed + PID_value;

  // The motor speed !> max PWM value
  ...
  //code to make the bot move forward
  forward();
}
